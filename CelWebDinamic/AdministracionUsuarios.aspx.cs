...
namespace CelWebDinamic
{
    public partial class AdministracionUsuarios : System.Web.UI.Page
    {
        ...
        private void cargarGrid(bool ResetPage = false)
        {
            ...
            if (!(string.IsNullOrEmpty(txtBusqueda.Text) && string.IsNullOrWhiteSpace(txtBusqueda.Text)) && txtBusqueda.Text.Length > 1)
            {
              
                gridUsuarios.DataSource = BL_Usuarios.vUsuarios().Where(a=>a.NombreCompleto.ToLower().Contains(txtBusqueda.Text.ToLower()) || a.Correo.ToLower().Contains(txtBusqueda.Text.ToLower()) || a.UserName.ToLower().Contains(txtBusqueda.Text.ToLower())).ToList();
            ...
            try
            {
                ...
                if (IdRegistro > 0)
                {
                    ...
                    if (validarActualizar(IdRegistro))
                    {
                        ...
                        User.IdUsuarioActualizado = IdUsuarioSistema;
                ...
                if (validarInsertar())
                {
                    ...
                    User.Password = BL_Usuarios.Encrypt(txtContraseña.Text);
                    User.IdRol = (int)General.ValidarEnteros(ddlRol.SelectedValue);
                    User.IdUsuarioRegistro = IdUsuarioSistema;
